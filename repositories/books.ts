...
class BookRepository {
  static create = async (data: BookCreate) => {
    try {
      if (data.isbn) {
  ...
  static findManyAndCountByFilter = async (
      ...
      const columns = selectFields?.length
        ? sql`${sql.join(
            selectFields.map(field => {
              const column = dsgBooks[field as keyof typeof dsgBooks];
              if (!column)
    ...
    `;
      ...
      const {results, total} = await db.transaction(async tx => {
        const results: Book[] = await tx.execute(query);
        const countResult: {count: number}[] = await tx.execute(countQuery);
        return {
          results,
